# Techniek — pipeline, kwaliteit en workflow

Deze pagina bevat de technische kant van de SV-modernisatie:
installatie, de architectuur, de kwaliteitscontroles, de skills en de
git-workflow. Voor wat het project *is* en hoe je de tekst leest, zie de
[README](README.md).

> **Leidend bij twijfel:** [`MODERNISATIE.md`](MODERNISATIE.md)
> (definitie, methode, aannames) en [`AGENTS.md`](AGENTS.md)
...
- [Viewer lokaal draaien](#viewer-lokaal-draaien)
...
Alle detaildocumentatie die de skills nodig hebben staat in losse
markdown-bestanden, zodat ze geen contextruimte afsnoepen van
`AGENTS.md`:
...
- [`WORKTREE_WORKFLOW.md`](WORKTREE_WORKFLOW.md) — parallelle sessies via git worktrees
- [`REGELBESTANDEN.md`](REGELBESTANDEN.md) — overzicht van de regelbestanden
...
Volledig schema en incrementeel gedrag: [`OUTPUT_SCHEMA.md`](OUTPUT_SCHEMA.md).
...
Conflict-detectie is volledig self-contained en draait via [wt_clash.py](scripts/wt_clash.py) (wat de noodzaak voor een externe `clash`-binary vervangt).
...
Volledig: [`WORKTREE_WORKFLOW.md`](WORKTREE_WORKFLOW.md).
